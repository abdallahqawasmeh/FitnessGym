...
namespace MyGymSystem.Context
{
    ...
    public class GymDbContext : DbContext
    {
        ...
        public DbSet<Trainerspage> Trainerspages { get; set; }
        public DbSet<Membershipplanspage> Membershipplanspages { get; set; }
        public DbSet<Testimonialspage> Testimonialspages { get; set; }

        // Updated based on your current fluent config :contentReference[oaicite:0]{index=0}
        //
        // ✅ What’s NEW / CHANGED (all changes are commented in-place):
        // 1) Added Subscription ↔ Member (FK) + Subscription ↔ Membershipplan (FK)  ✅ NEW
        // 2) Added Subscription ↔ Trainer (chosen trainer for Personal/Private plan) ✅ NEW
        // 3) Added helpful indexes for Subscription.Memberid / Planid / Trainerid    ✅ NEW
        // 4) (Optional but recommended) Added MemberTrainer ↔ Subscription link      ✅ NEW (requires adding SubscriptionId in MemberTrainer model)
        // 5) Improved MemberTrainer “one active trainer per member” index to the cleanest version ✅ CHANGED (still SQL Server filtered)
        // 6) Kept ALL your existing comments + added comments for every new/changed part
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            // ======================================================================
            // ✅ NEW (IMPORTANT): Subscription ↔ Member / Plan / Trainer relations
            // ======================================================================
            // Why we add these?
            // - Member subscribes to a plan -> Subscription must point to Member & Plan
            // - For Personal/Private plans -> Subscription MUST store the chosen TrainerId
            //   (so trainer can later see who chose him from subscriptions)
            //
            // NOTE: This requires you to add these to Subscription model:
            // public long? Trainerid { get; set; }
            // public virtual Trainer? Trainer { get; set; }
            //
            // Recommended (best practice): make Memberid + Planid NOT nullable in Subscription model.

            modelBuilder.Entity<Subscription>()
     .HasOne(s => s.Member)
     .WithMany(m => m.Subscriptions)
     .HasForeignKey(s => s.Memberid)
     .OnDelete(DeleteBehavior.Restrict); // ✅ CHANGED: avoid SQL Server multiple cascade paths + protect financial history

            modelBuilder.Entity<Subscription>()
                .HasOne(s => s.Plan)
                .WithMany(p => p.Subscriptions)
                .HasForeignKey(s => s.Planid)
                .OnDelete(DeleteBehavior.Restrict); // Prevent deleting a plan that has subscriptions

            modelBuilder.Entity<Subscription>()
     .HasOne(s => s.Trainer)
     .WithMany(t => t.Subscriptions) // ✅ CHANGED: you already have Trainer.Subscriptions, so map it explicitly to avoid Trainerid1
     .HasForeignKey(s => s.Trainerid)
     .OnDelete(DeleteBehavior.Restrict); // Prevent deleting trainer while linked to subscriptions

            // ✅ NEW indexes for common queries:
            // - Member history: filter by Memberid
            // - Plan usage: filter by Planid
            // - Trainer dashboard: filter by Trainerid (members who chose him)
            modelBuilder.Entity<Subscription>().HasIndex(s => s.Memberid);
            modelBuilder.Entity<Subscription>().HasIndex(s => s.Planid);
            modelBuilder.Entity<Subscription>().HasIndex(s => s.Trainerid);
            ...
            // ======================================================================
            // ✅ MemberTrainer relations (Member chooses Trainer for Personal plan)
            // ======================================================================
            ...
            // ======================================================================
            // ✅ NEW (Recommended): Link MemberTrainer assignment to the Subscription that created it
            // ======================================================================
            // Why?
            // - You can audit: "this member was assigned to this trainer because of subscription X"
            // - When subscription ends/cancelled, you can deactivate assignment and still keep history
            //
            // NOTE: This requires you to add to MemberTrainer model:
            // public long? SubscriptionId { get; set; }
            // public virtual Subscription? Subscription { get; set; }

            modelBuilder.Entity<MemberTrainer>()
                .HasOne(mt => mt.Subscription)
                .WithMany()
                .HasForeignKey(mt => mt.SubscriptionId)
                .OnDelete(DeleteBehavior.SetNull);

            // Enforce 1 active trainer per member (filtered unique index; SQL Server)
            // ✅ CHANGED (cleaner than composite index):
            // - Only ONE row can exist where IsActive = 1 for a specific MemberId
            modelBuilder.Entity<MemberTrainer>()
                .HasIndex(mt => mt.MemberId)
                .IsUnique()
                .HasFilter("[IsActive] = 1");

            // ✅ NEW index: speed up trainer dashboard queries (who is assigned to me)
            modelBuilder.Entity<MemberTrainer>()
                .HasIndex(mt => mt.TrainerId);

            // ✅ NEW index: speed up tracing assignment by subscription
            modelBuilder.Entity<MemberTrainer>()
                .HasIndex(mt => mt.SubscriptionId);
            ...
            // =====================================================
            // ✅ NEW CMS Pages (Trainers/Plans/Testimonials) ↔ Admin
            // =====================================================

            modelBuilder.Entity<Trainerspage>()
                .HasOne(p => p.Updatedbyadmin)
                .WithMany(a => a.Trainerspages) // ✅ add ICollection in Admin
                .HasForeignKey(p => p.Updatedbyadminid)
                .OnDelete(DeleteBehavior.SetNull);

            modelBuilder.Entity<Membershipplanspage>()
                .HasOne(p => p.Updatedbyadmin)
                .WithMany(a => a.Membershipplanspages) // ✅ add ICollection in Admin
                .HasForeignKey(p => p.Updatedbyadminid)
                .OnDelete(DeleteBehavior.SetNull);

            modelBuilder.Entity<Testimonialspage>()
                .HasOne(p => p.Updatedbyadmin)
                .WithMany(a => a.Testimonialspages) // ✅ add ICollection in Admin
                .HasForeignKey(p => p.Updatedbyadminid)
                .OnDelete(DeleteBehavior.SetNull);
